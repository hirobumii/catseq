...
def test_rwg_device_init_invalid_config():
    ...
    """
    with pytest.raises(
        ValueError, match="Policy allows ramping, but hardware does not support it"
    ):
        RWGDevice(
            name="bad_rwg", available_sbgs={0}, max_ramping_order=0, allow_ramping=True
        )
...
def test_validate_dynamics_unavailable_sbg(rwg_device):
    ...
    dynamics = (
        WaveformParams(sbg_id=99, freq_coeffs=(1, 0, 0, 0), amp_coeffs=(1, 0, 0, 0)),
    )
...
def test_validate_dynamics_ramping_not_allowed(rwg_device):
    ...
    dynamics = (
        WaveformParams(sbg_id=0, freq_coeffs=(1, 1, 0, 0), amp_coeffs=(1, 0, 0, 0)),
    )
...
def test_validate_dynamics_order_exceeds_capability(rwg_device):
    ...
    dynamics = (
        WaveformParams(sbg_id=0, freq_coeffs=(1, 1, 1, 0), amp_coeffs=(1, 0, 0, 0)),
    )
    with pytest.raises(
        TypeError, match="Required order is 2, but hardware only supports up to 1"
    ):
        rwg_device.validate_dynamics(dynamics)
