...
def simple_builder(
    name: str, from_state: State, to_state: State, duration: float
) -> MorphismBuilder:
...
def test_primitive_post_init_validation(ch_a, ch_b):
    ...
    with pytest.raises(ValueError, match="channel must be consistent"):
        PrimitiveMorphism(
            "Inconsistent", dom=((ch_a, StateA()),), cod=((ch_b, StateB()),), duration=1
        )
