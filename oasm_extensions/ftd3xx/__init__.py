from . import _ftd3xx_win32 as _ft
import ctypes as c
from .defines import *
...
class FTD3XX(object):
    ...
    def initOverlapped(self):
        ovlp = _ft.OVERLAPPED()
        self.status = call_ft(_ft.FT_InitializeOverlapped, self.handle, c.byref(ovlp))
        return ovlp
    
    def releaseOverlapped(self, ovlp):
        self.status = call_ft(_ft.FT_ReleaseOverlapped, self.handle, c.byref(ovlp))
        return None
    
    def getOverlappedResult(self, ovlp):
        bytesTransferred = _ft.ULONG()
        self.status = call_ft(_ft.FT_GetOverlappedResult, self.handle, c.byref(ovlp), c.byref(bytesTransferred), _ft.BOOL(True))
        return bytesTransferred.value

    def asyncReadPipe(self, pipe, datalen, ovlp):
        bytesTransferred = _ft.ULONG()
        data = c.c_buffer(datalen)
        self.status = call_ft(_ft.FT_ReadPipe, self.handle, _ft.UCHAR(pipe),
                                c.byref(data), _ft.ULONG(datalen), c.byref(bytesTransferred), c.byref(ovlp))
        return data
    ...
    def readPipeEx(self, pipe, datalen):
        """Recv the data to the device."""
        bytesTransferred = _ft.ULONG()
        data = c.c_buffer(datalen)
        self.status = call_ft(_ft.FT_ReadPipe, self.handle, _ft.UCHAR(pipe), data, _ft.ULONG(datalen), c.byref(bytesTransferred), None)
        return (bytesTransferred.value, data.raw[:bytesTransferred.value])
