...
def test_linear_ramp_uses_previous_state_when_materialized():
    ...
    realized, end_states = rwg_v2.linear_ramp(target, 5 * us).on(ch).materialize_with_states(start)

    ops = tuple(timed.operation for timed in realized.timed_operations())
    assert len(ops) == 5
...
def test_initialize_materializes_from_uninitialized_like_state_chain():
    ...
    realized, end_states = rwg_v2.initialize(120.0).on(ch).materialize_with_states(RWGUninitialized())

    assert {timed.operation.channel for timed in realized.timed_operations()} == {ch}
