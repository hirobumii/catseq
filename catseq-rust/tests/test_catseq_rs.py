...
class TestNode:
    ...
    def test_atomic_creation(self):
        """测试创建原子节点"""
        ctx = catseq_rs.CompilerContext()
        node = ctx.atomic(0, 100, b"test_payload")
    ...
    def test_sequential_composition(self):
        """测试串行组合 (@)"""
        ctx = catseq_rs.CompilerContext()
        n1 = ctx.atomic(0, 100, b"op1")
        n2 = ctx.atomic(0, 50, b"op2")
    ...
    def test_parallel_composition(self):
        """测试并行组合 (|)"""
        ctx = catseq_rs.CompilerContext()
        n1 = ctx.atomic(0, 100, b"op1")  # channel 0
        n2 = ctx.atomic(1, 200, b"op2")  # channel 1
    ...
    def test_parallel_channel_conflict(self):
        ...
        ctx = catseq_rs.CompilerContext()
        n1 = ctx.atomic(0, 100, b"op1")  # channel 0
        n2 = ctx.atomic(0, 50, b"op2")   # channel 0 (conflict!)
...
class TestCompilation:
    ...
    def test_compile_sequential(self):
        """测试编译串行组合"""
        ctx = catseq_rs.CompilerContext()
        n1 = ctx.atomic(0, 100, b"first")
        n2 = ctx.atomic(0, 50, b"second")
        seq = n1 @ n2
    ...
    def test_compile_parallel(self):
        """测试编译并行组合"""
        ctx = catseq_rs.CompilerContext()
        n1 = ctx.atomic(0, 100, b"ch0")
        n2 = ctx.atomic(1, 50, b"ch1")
        par = n1 | n2
    ...
    def test_compile_complex(self):
        ...
        b = ctx.atomic(1, 50, b"B")
        c = ctx.atomic(0, 30, b"C")
    ...
    def test_compile_by_board(self):
        ...
        n1 = ctx.atomic(0, 100, b"board0")
        n2 = ctx.atomic(1, 100, b"board0_ch1")
        par = n1 | n2
...
class TestPerformance:
    ...
    def test_deep_chain(self):
        ...
        root = ctx.atomic(0, 1, b"op0")
        for i in range(1, 1000):
            next_op = ctx.atomic(0, 1, b"op")
            root = root @ next_op
    ...
    def test_wide_parallel(self):
        ...
        nodes = []
        for i in range(50):
            nodes.append(ctx.atomic(i, 10 * (i + 1), f"op{i}".encode()))
...
class TestEdgeCases:
    ...
    def test_large_payload(self):
        ...
        node = ctx.atomic(0, 100, large_payload)
        events = node.compile()
        assert events[0][2] == large_payload

    def test_zero_duration(self):
        """测试零时长"""
        ctx = catseq_rs.CompilerContext()
        node = ctx.atomic(0, 0, b"instant")
        assert node.duration == 0

    def test_clear_arena(self):
        """测试清空 Arena"""
        ctx = catseq_rs.CompilerContext()
        ctx.atomic(0, 100, b"test")
        assert ctx.node_count() == 1
